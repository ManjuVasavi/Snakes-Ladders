...
public class Game{
    ...
    private void initBoard(){
        ...
        for(int i=0;i<snakesno;i++){
            while(true){
                ...
                int snakeStart=ran.nextInt(board.getSize());
                int snakeEnd=ran.nextInt(board.getSize());
        ...
        for(int i=0;i<laddersno;i++){
            while(true){
                Random ran=new Random();
                int ladderStart=ran.nextInt(board.getSize());
                int ladderEnd=ran.nextInt(board.getSize());
    ...
    public void playGame(){
        while(true){
            ...
            int val=dice.roll();
            System.out.println("Dice Roll Value: "+val);
            ...
            if(newPosition>board.getEnd()){
                ...
                if(player.getPositon()==board.getEnd()){
                    ...
                    System.out.println("Setting "+player.getName()+"'s new position to "+player.getPositon());
                    players.offer(player);
                }
            }
            if(players.size()<2){
